...
def login():
    ...
    global server_bot
    server_bot = mineflayer.createserver_bot({"host": '109.248.206.65', "port": 25565, "username": 'fleshka', "version": '1.18.2'})
    sleep(10)
    server_bot.chat('/l 123123p')
    sleep(2)

def get_tab_data():
    header = server_bot.tablist.header.json.valueOf()
    footer = server_bot.tablist.footer.json.valueOf()
    players = server_bot.players.valueOf()
    return header, players, footer

def draw_colored_text(header, footer, players, font_path, output_image, background_image_path=None):
        ...
        for line in text_lines:
            ...
            for word in words:
                if len(word) > 0:
                    ...
                    if style_code in color_map:
                        current_color = color_map[style_code]
                    elif style_code in style_map:
                        current_style = style_map[style_code]
                    elif style_code == 'r':
                        current_color = (0, 0, 0)
                        current_style = ''
        ...
        for player_data in players[player]['displayName']['json']:
            if player_data == 'extra':
                ...
                for stat in players[player]['displayName']['json'][player_data]:
                    tag = ''
                    if "color" in stat:
                        tag = '&' + colors[stat['color']] + stat['text']
                    else:
                        tag = '&f' + stat['text']
                    if 'bold' in stat:
                        if stat['bold'] == True:
